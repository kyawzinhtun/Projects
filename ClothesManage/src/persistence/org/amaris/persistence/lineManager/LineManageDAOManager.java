...
public class LineManageDAOManager extends SqlMapClientDaoSupport implements
		ILineManageDAOManager {
	...
	public void insertAcc(String id, List<StockView> accessoriesList) {
		...
		try {
			for (StockView sv : accessoriesList) {
				sv.setLineId(id);
				StockView stockQty = (StockView) getSqlMapClientTemplate()
						.queryForObject("lineManager.selectStockViewByQtyId",
								Integer.parseInt(sv.getStockQtyId()));
				int remainQty = Integer.parseInt(stockQty.getStockQuantity())
						- Integer.parseInt(sv.getStockQuantity());
				Double totalPrice = (double) (remainQty * Integer
						.parseInt(stockQty.getStockPrice()));
				sv.setStockQuantity(String.valueOf(remainQty));
				sv.setTotalPrice(String.valueOf(totalPrice));
				getSqlMapClientTemplate().insert(
						"lineManager.insertAccessories", sv);
				getSqlMapClientTemplate().update("lineManager.updateStockView",
						sv);
			}
		} catch (DataAccessException de) {
			de.printStackTrace();
	...
	public List<Line> selectAllCompleteLine(String status) {
		...
		try {
			resultList = getSqlMapClientTemplate().queryForList(
					"lineManager.selectCompleteLine", status);
		} catch (DataAccessException de) {
			de.printStackTrace();
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public List<Line> selectAllUnCompleteLine(String status) {
		...
		try {
			resultList = getSqlMapClientTemplate().queryForList(
					"lineManager.selectUnCompleteLine", status);
		} catch (DataAccessException de) {
			de.printStackTrace();
		} catch (Exception ex) {
			ex.printStackTrace();
		...
		try {
			for (Line line : lineList) {
				getSqlMapClientTemplate()
						.update("lineManager.updateLine", line);
			}
		} catch (DataAccessException de) {
			de.printStackTrace();
		} catch (Exception ex) {
	...
	public List<StockView> selectAll() {
		...
		try {
			resultList = getSqlMapClientTemplate().queryForList(
					"lineManager.selectAllStockView");
		} catch (DataAccessException de) {
			de.printStackTrace();
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public List<StockView> selectById(String id) {
		...
		try {
			resultList = getSqlMapClientTemplate().queryForList(
					"lineManager.selectStockViewById", id);
		} catch (DataAccessException de) {
			de.printStackTrace();
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public List<Accessories> getAccessoriesListById(String id) {
		...
		try {
			resultList = getSqlMapClientTemplate().queryForList(
					"lineManager.selectAccessoiresById", id);
		} catch (DataAccessException de) {
			de.printStackTrace();
		} catch (Exception ex) {
			ex.printStackTrace();
