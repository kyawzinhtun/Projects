...
public class SupplierSearchFrame extends JFrame {
	...
	public static SupplierSearchFrame supplierSearchFrame;
	private ISupplierServiceManager supplierSerivceManger;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					SupplierSearchFrame frame = new SupplierSearchFrame();
	...
	public static SupplierSearchFrame getInstance() {
		if (supplierSearchFrame == null) {
			supplierSearchFrame = new SupplierSearchFrame();
			supplierSearchFrame.setExtendedState(JFrame.MAXIMIZED_BOTH); 
			supplierSearchFrame.setVisible(true);
			supplierSearchFrame.setResizable(false);
			return supplierSearchFrame;
		}
		supplierSearchFrame.setExtendedState(JFrame.MAXIMIZED_BOTH); 
		supplierSearchFrame.setVisible(true);
		supplierSearchFrame.setResizable(false);		
	 ...
	 * Create the frame.
	 */
	public SupplierSearchFrame() {
		
		supplierSerivceManger = (ISupplierServiceManager) BasicBeanFactory.getBean("SupplierServiceManager");
		...
		JLabel lblLogo = new JLabel("Supplier Searching");
		...
		lblHome.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				supplierSearchFrame.setVisible(false);
				supplierSearchFrame = null;
			}
			@Override
		...
		lblAdd.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				SupplierManage supplierFrame = SupplierManage.getInstance();
				supplierSearchFrame.setVisible(false);
				supplierSearchFrame = null;
			}
			@Override
	...
	class CheckBoxHandler implements MouseListener {
		...
		public void mouseClicked(MouseEvent e) {
			 ...
			 if(selectedCol==5) {
				 Conversation.addConversation("supplier_Detail", dtm.getValueAt(selectedRow, 1)+"");
				 
				// Conversation.addConversation("supplier_Detail","000001");
				 SupplierDetailFrame supplierDetailFrame = SupplierDetailFrame.getInstance();
				 supplierSearchFrame.setVisible(false);
				 supplierSearchFrame = null;
			 }
    ...
    public void initializedDate() {
    	...
    	List<Supplier> resultList = new ArrayList<Supplier>();
			resultList = supplierSerivceManger.findAll();
			int i=0;
			for(Supplier supplierObject : resultList) {
				dtm.addRow(new Object[]{
						i+1,
						supplierObject.getId(),
						supplierObject.getSupplierName(),
						supplierObject.getSupplierAddress(),
						supplierObject.getSupplierPh(),
						"Detail",
				});
				i++;
			}
	...
	public void searchStockFilter(String filterKey) {
		...
		if(StringUtils.isBlank(filterKey)){
			...
			try {
				resultList = supplierSerivceManger.findByNameFilter(filterKey);
				int i = 0;
				removeAllRow();
					for(Supplier supplierObject : resultList) {
						dtm.addRow(new Object[]{
